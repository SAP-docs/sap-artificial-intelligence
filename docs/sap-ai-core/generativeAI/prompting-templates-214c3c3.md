...
For security reasons, the system prompt is predefined and managed by SAP. Custom system prompts are not supported.

If a system prompt is included in the request, the request fails and an error is returned.

In typical scenarios, providing a user prompt is sufficient. If additional instructions are required, include them at the beginning or end of the user prompt.
