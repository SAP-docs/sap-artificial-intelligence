...
### Gemini 2.0 Flash | Gemini 2.0 Flash Lite | Gemini 2.5 Flash | Gemini 2.5 Flash Lite | Gemini 2.5 Pro | Gemini 3.1 Flash Lite | Gemini 3.5 Flash | Gemini-3.1 Flash Image | Gemini 3 Pro Image| Gemini 3.5 Flash Lite
...
```
curl --request POST --location "$DEPLOYMENT_URL/models/gemini-2.5-flash-image:generateContent" \
  ...
  "contents": [
    {
      ...
      "parts": [
        {
          "text": "A cozy, cinematic coffee shop interior at golden hour, shallow depth of field, 35mm, soft rim light"
        }
      ]
    }
  ]
...
```
curl --location "$DEPLOYMENT_URL/models/gemini-embedding-2:embedContent" \
--header "AI-Resource-Group: $AI_RESOURCE_GROUP" \
--header 'Content-Type: application/json' \
--header "Authorization: Bearer $AUTH_TOKEN" \
--data '{
...
-   Compatible Formats: JPEG, PNG, WebP, BMP, HEIC, HEIF, AVIF
-   Maximum 6 images per request.
-   Maximum 16384 x 16384 pixels per image
-   Maximum 1 file per request, of maximum length 6 pages.

    For best quality, use 1 page per PDF.
...
Audio must meet the following criteria:

-   Audio Limits: 180 seconds per request.
-   Format: MP3, WAV

Inputs longer than 8192 tokens are silently truncated. All modalities share the total 8192 tokens context window.
...
-   Compatible Formats: MOV, MP4. Codec: AV1, H264, H265, VP9.
-   Maximum 1 video per request, of up to 120 frames

-   At the default 1 FPS setting the length of the video can be up to 120 seconds. Duration varies when you set different FPS: Duration = Frames/FPS. By default, audio extraction is disabled for video. When the audio track extraction option is enabled, the duration is subject to the maximum audio duration of 180 seconds, and the total video duration is limited by both the audio limit and the token context window \(8192 tokens\).
