...
To run an optimization, you must provide a test dataset in JSON format. Your test dataset must contain data needed for the optimization, such as values for the prompt variables and ground truth references.
...
If you want to use train and test datasets, use the following folder structure.
...
Send a POST request to the endpoint `$AI_API_URL/v2/lm/artifacts`, and include the prompt optimization label and the path to your artifact within your object store in your request.
...
Create a configuration for your optimization. For more information, see [Create a Configuration for a Prompt Optimization](create-a-configuration-for-a-prompt-optimization-40ba168.md).
