...
If both prompt attack detection and harm classification are configured, the orchestration service performs the prompt attack detection call first, and then performs the harm classification call. If a prompt attack is detected, the orchestration service doesn't make the harm classification request and returns the prompt attack detection result only.

> ### Note:  
> Content from messages with the `system` and `developer` roles is excluded from the `prompt_shield` content filter. As a result, if a request contains only `system` or `developer` messages, the `prompt_shield` content filter is skipped because there's no content to evaluate.
...
> ### Remember:  
> When multiple content filtering types are configured together, each type is processed as a separate request. Costs are incurred for each filtering type applied.

> ### Note:  
> To ensure continued service availability, the orchestration service supports regional fallbacks for the Azure Content Safety service. If a request fails due to service unavailability, the orchestration service automatically retries the request using a fallback endpoint within the same region.
...
> ### Note:  
> Llama Guard 3 8B can report violations for categories that haven't been requested by the user
> 
> The orchestration service doesn't consider these violations when processing Llama Guard responses, and only filters violations in categories that have been requested by the user.
