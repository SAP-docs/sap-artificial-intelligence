...
Orchestration supports the model parameters offered by OpenAI, including `max_completion_tokens`, `temperature`, `frequency_penalty`, `presence_penalty`, `n`, and `top_p`. Where possible, these parameters are used with any model and mapped to the corresponding parameter of the foundation model. For example, OpenAI's `max_completion_tokens` will be mapped to Vertex AI's `maxOutputTokens`. Additionally, any model-specific parameters can be sent. However when using model specific parameters they should be in snake\_case format for consistency. For example, you can use Vertex AI's `topK` parameter as `top_k`, which is normally not available in OpenAI's API.
...
[Access the Harmonized API using the SAP AI SDK for Java](https://sap.github.io/ai-sdk/docs/java/orchestration/chat-completion#harmonized-api)

[Access the Harmonized API using the SAP AI SDK for JavaScript](https://sap.github.io/ai-sdk/docs/js/orchestration/chat-completion#harmonized-api)
