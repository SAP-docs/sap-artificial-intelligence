...
To use a model through the `foundation-models` scenario, you must create a configuration and then a deployment. You need the `executableId`: `aicore-sap` and `modelName`: `sap-rpt-1-small` or `sap-rpt-1-large`. You will also need a model version, or you can use `latest` to use the latest version. For more information, see [Create a Deployment](create-a-deployment-b32e7a8.md).
