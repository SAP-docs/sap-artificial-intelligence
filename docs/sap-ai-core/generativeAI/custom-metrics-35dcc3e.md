...
LLM-as-a-judge-type metrics are supported. A custom LLM-as-a-judge metric uses a judge LLM with a rubric to compute a metric rating. The output can be in numeric or text format.

You can define custom LLM-as-a-judge metrics using structured prompts.
...
Structured prompts are the recommended way to define custom metrics. They use a predefined format that you can use to define the metrics.
