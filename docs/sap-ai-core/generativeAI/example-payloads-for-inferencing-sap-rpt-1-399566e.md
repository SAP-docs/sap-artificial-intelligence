...
`sap-rpt-1-small` and `sap-rpt-1-large` accept tabular data requests in the following formats:
...
If omitted, a regression task is assumed if at least one value in the input data is formatted like a float, \(i.e. `"1.0"`\), otherwise classification is performed.
...
A schema definition for data types of all columns, for example `{"columnA": {"dtype": "string"}, "columnB": {"dtype": "numeric"}, ...}`
...
If the `index_column` parameter has been passed, the contents of this column are also included in the response.
...
Confidence value for this classification prediction \(null for regression\)
...
predictions\[\].\*\[\].confidence\_interval
...
Confidence interval for this regression prediction as array of two floats `[lower_bound, upper_bound]` or `null` for classification
            ...
            "salary": [
                {
                    ...
                    "confidence": null,
                    "confidence_interval": [50000.0, 58500.0]
                }
            ]
        }
    ],
