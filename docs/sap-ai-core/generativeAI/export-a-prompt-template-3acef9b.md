...
Send a GET request to the endpoint `$AI_API_URL/v2/lm/promptTemplates/{{promptTemplateId}}/export`, and include the name of your file in the `output` parameter.

 > ### Sample Code:  
> ```
> curl -X GET "$AI_API_URL/v2/lm/promptTemplates/{{promptTemplateId}}/export" \
