...
Send POST request to the endpoint `$ORCHESTRATION_DEPLOYMENT_URL/v2/embeddings` and populate the sample code with the following values:
...
$ORCHESTRATION\_DEPLOYMENT\_URL
...
```
curl --request POST \
  --url $ORCHESTRATION_DEPLOYMENT_URL/v2/embeddings \
...
> ### Note:  
> For most models this parameter is optional or not supported. For `nvidia--llama-3.2-nv-embedqa-1b``input_types` is mandatory.
> 
...
[Access the Embedding using the SAP AI SDK for Java](https://sap.github.io/ai-sdk/docs/java/orchestration/embedding)

[Access the Embedding using the SAP AI SDK for JavaScript](https://sap.github.io/ai-sdk/docs/js/orchestration/embedding)
