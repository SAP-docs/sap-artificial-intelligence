...
`metricType` \(optional\) \(deprecated\)

> ### Tip:  
> `metricType` is deprecated, therefore we recommend using `usageType` instead.
...
> ### Restriction:  
> For prompt optimization tasks: Only custom metrics that use the LLM-as-a-judge evaluation method and have numerical or Boolean output types are supported.
...
A list containing the purpose of the metric. The list can be empty, or contain one or more of the following:
...
Your response includes one of the following response codes:
...
Successful responses include a unique ID that can be used to retrieve your custom metric later.
