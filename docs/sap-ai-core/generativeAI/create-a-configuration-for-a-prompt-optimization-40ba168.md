...
-   You've registered an object store secret for input artifacts. For more information, see [Register an Object Store for Optimizations](register-an-object-store-for-optimizations-54068a9.md).
...
-   If your prompt template variable names and test data attribute names are mismatched, you have prepared your variable mapping. For more information, see [Variable Mapping](variable-mapping-bb3d4f8.md).
...
A JSON string specifying the model that the parameters should be applied to, and a comma separated list of parameters and parameter values. The following parameters are supported:

-   `temparature`\(0-1\)
-   `max_tokens` \(0-n \(int\)\)

Any unsupported variables passed will be ignored.
...
-   `FC:ExactMatch`
    ...
    A variation on `ExactMatch` specifically for use cases that include function calling.

-   `BLEU`

    This metric evaluates machine-translated text quality by calculating n-gram precision between candidate and reference translations. Scores range from 0 to 1, with higher values indicating greater similarity.

-   `ROUGE`

    A set of metrics for evaluating summarization and machine translation by measuring overlap in n-grams, word sequences, and word pairs between generated and reference texts. This implementation is case-insensitive.

-   `METEOR`

    This metric evaluates response quality using a combination of precision, recall, and synonym matching. Better handles morphological variations and paraphrasing compared to BLEU. Particularly useful for languages with rich morphology.

-   `EXACT_MATCH`

    A Boolean indicating whether the output exactly matches the reference.
...
A JSON containing column name mappings. For more information, see [Variable Mapping](variable-mapping-bb3d4f8.md).
...
>         {
>             "key": "modelParams",
>             "value": "{"<modelName1>:<modelVersion1>":{"temperature":0.7,"max_tokens":1000},"<modelName2>:<modelVersion2>":{"temperature":0.9}}"
>         },
>         {
>             "key": "dataset",
>             "value": "data.json"
...
>         {
>             "key": "variableMapping",
>             "value": "{"<prompt_variable1>":"<data_variable1>"}"
>         },
>         {
