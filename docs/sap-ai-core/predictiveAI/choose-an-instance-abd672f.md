...
You must choose at least one instance. If you select an instance type, a resource plan isn't required. Similarly, if you select a resource plan, an instance type isn't required.
...
Instance types and resource plans are used to select instances in workflow and serving templates. Different steps of a workflow can have different instances assigned.
...
There are limits to the default disk storage size for all these nodes. Data sets that are loaded to the nodes consume disk space. If you have large data sets \(larger than 30 GB\), or have large models, consider increasing your disk size. To do so, use the persistent volume claim in Argo Workflows to specify the required disk size \(see [Volumes](https://argoproj.github.io/argo-workflows/walk-through/volumes/)\).
...
`$AI_API_URL/v2/admin/resources/instanceTypes`
...
> ### Sample Code:  
> ```
> curl GET "$AI_API_URL/v2/admin/resources/instanceTypes" \
