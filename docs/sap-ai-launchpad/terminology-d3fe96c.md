...
A way of binding values \(for datasets or models and input parameters\) with a specific version of an executable/ template.
...
-   A serving executable/deployment template defines the expected parameters and input dataset that is required for the serving process.
...
A template that is used to serve/deploy one or more trained models.
...
For more information, see [Templates](https://help.sap.com/viewer/2d6c5984063c40a59eda62f4a9135bee/CLOUD/en-US/8a1f91a18cf0473e8689789f1636675a.html), [Executables](executables-06f4794.md), or [Templates](templates-442a7e8.md).
...
Placeholder in an executable/template that enables datasets or models to be attached.
...
The name of the artifacts \(usually AI models\) that are generated by an execution / run.
...
In the SAP AI Launchpad, a scenario consists of executables /templates that you use to implement the AI use case within a service. A configuration is used to attach values to the placeholders.
...
Scenarios and their executables/ templates are available to all AI scenario consumers.
